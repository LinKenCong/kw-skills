...
- Read `agent-brief.json` first for each repair round. Read `text-manifest.json` before editing visible copy; it is the authoritative Figma text source.
...
4. Read `agent-brief.json`, `text-manifest.json`, and the relevant React/CSS files.
