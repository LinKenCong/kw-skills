...
`service start` binds to the fixed loopback service port `49327`. The development plugin manifest and UI use `http://localhost:49327` because Figma validates `localhost` for development network access; the CLI lockfile may still report `http://127.0.0.1:49327`. V1 does not expose a custom port in the CLI. It does not hot-reload the Figma plugin UI/main script; relaunch or re-import the development plugin after changing files under `plugin/`.
